...
def render_content():
    ...
    # 1. Inyectamos CSS para crear el contenedor pegajoso (sticky)
    st.markdown(
    """
    <style>
        /* 1. Contenedor principal Sticky */
        div[data-testid="stVerticalBlock"] > div:has(span.sticky-filter-marker) {
            position: sticky;
            top: 2.875rem;          /* Altura bajo la barra de Streamlit */
            z-index: 999;
            
            /* Color de fondo exacto del tema de la app (evita transparencias sin meter colores extras) */
            background-color: var(--background-color, #ffffff);
            
            /* Separación interna y linea sutil inferior */
            padding-top: 0.5rem;
            padding-bottom: 0.8rem;
            margin-bottom: 1rem;
            border-bottom: 1px solid rgba(0, 0, 0, 0.08); /* Línea divisoria muy suave */
        }

        /* 2. Ajuste para el Expander dentro del Sticky */
        div[data-testid="stVerticalBlock"] > div:has(span.sticky-filter-marker) div[data-testid="stExpander"] {
            /* Mantiene el fondo por defecto del expander (sin color forzado) */
            background-color: var(--background-color, #ffffff);
            border-radius: 8px;
            box-shadow: 0px 2px 8px rgba(0, 0, 0, 0.05); /* Sombra muy sutil */
        }

        /* 3. Oculta el contenedor del marcador para que no cree espacios fantasma */
        div[data-testid="stElementContainer"]:has(span.sticky-filter-marker) {
    ...
    # Contenedor para los filtros de la pestaña
    with st.container():
        st.markdown(
        '<span class="sticky-filter-marker"></span>', unsafe_allow_html=True
        )
       #st.subheader("Filtros de consulta")
       # Expander que permanece fijo al hacer scroll
        with st.expander("🔍 **Filtros de consulta**", expanded=True):
            with st.form(key="QueryForm"):
                # Fila 1: Selección de contaminante y estación
                ...
                with col1:
                        pollutant_sel = st.selectbox(
                            ...
                            key="hist_pollutant_new",
                        )

                    # Filtrado dinámico de estaciones según el contaminante
                codigos_validos = [
                        cod
                        for cod, info in mc.ESTACIONES_MADRID.items()
                ...
                if fecha_inicio > fecha_fin:
                        ...
                        return  # Sale de la funcion para no intentar cargar las graficas con fechas invalidas

                # Fila 3: Parámetros meteorológicos
                opciones_meteo = {
                ...
                seleccionados_new = st.multiselect(
                        ...
                        key="seleccionados_new"
                        )
                st.markdown("---")

                # 2. BOTÓN DE ENVÍO DEL FORMULARIO (Obligatorio en st.form)
    ...
    if not df.empty:
        ...
        if not df.empty:
            ...
            def create_chart(y_field, title, color, is_last=False, add_threshold=False):
                ...
                points = base.mark_point(color=color, fill='black').encode(opacity=alt.condition(nearest, alt.value(1), alt.value(0)))
                rule = alt.Chart(df).mark_rule(color='black', strokeDash=[4,4]).encode(x='timestamp:T').transform_filter(nearest)
                
                text = base.mark_text(align='left', dx=8, dy=-8, color='black', fontWeight='bold').encode(
            ...
            for i, nombre in enumerate(seleccionados_new):
                es_ultimo = (i == len(seleccionados_new) - 1)
    ...
    colTopFive, colPatronHistorico = st.columns(2)
    with colPatronHistorico:
    ...
    with st.expander("🔍 **Mapa asignación sensores de tráfico**", expanded=False):
