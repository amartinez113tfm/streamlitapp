...
def render_content(pollutant_sel,codigo_sel,fecha_inicio,fecha_fin,seleccionados):
    ...
    with st.spinner("Conectando con Copernicus y procesando datos meteorológicos de Madrid..."):
        ...
        with col1:
            ...
            st_data_sat=st_folium(mapa_satelite, width=540, height=450, key="mapa_sat_key")

        with col2:
            st.markdown("#### 📍 Datos Terrestres (Estaciones en Tierra)")
            ...
            if df_tierra_listo.empty:
                ...
                st_folium(mapa_tierra_vacio, width=540, height=450, key="mapa_vacio_key")
            else:
                #mapa_tierra = cop.generar_mapa_estaciones(df_tierra_listo, pollutant_sel)
                st_data_tie=st_folium(mapa_tierra, width=540, height=450, key="mapa_tie_key")
    ...
    if chart_objeto is not None:
        ...
        col_grafica2, col_dataframe = st.columns([2, 3])
        
        
        with col_grafica2:
            st.markdown(f"📊 **Histórico de Superaciones ({pollutant_sel})**")
            evento_seleccion = st.altair_chart(chart_objeto, use_container_width=True, on_select="rerun")
            ...
            if isinstance(datos_clicados, list) and len(datos_clicados) > 0:
                ...
                if id_estacion_activa is not None:
                    ...
                    if fecha_concreta is not None:
                        st.markdown("---")
                        st.markdown(f"### 🔍 Diagnóstico Dinámico del Incidente: **{pd.to_datetime(fecha_concreta).strftime('%d/%m/%Y')}**")
